...
from pybo.forms import QuestionForm, AnswerForm # 폼 클래스 임포트
...
def _list():
    ...
    if kw:
        search = '%%{}%%'.format(kw)
        sub_query = db.session.query(Answer.questions_id, Answer.content, User.username)  \
                            .join(User, Answer.user_id == User.id).subquery()

        question_list = (question_list 
            .outerjoin(sub_query, sub_query.c.questions_id == Question.id)
    ...
    if so == 'recommend':
        ...
        question_list = (question_list 
            ...
            .order_by(func.count(distinct(question_voter.c.user_id)).desc(), 
                      Question.create_date.desc()))
    elif so == 'popular':
        # 인기순 정렬 (답변수 기준)
        question_list = (question_list 
            .outerjoin(Answer, Answer.questions_id == Question.id) 
            .group_by(Question.id) 
            .order_by(func.count(distinct(Answer.id)).desc(), 
                      Question.create_date.desc()))
    else:  # recent (최신순)
        question_list = (question_list 
            .group_by(Question.id) 
...
def create():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        question = Question(
            ...
            # user=g.user,image_path=image_path    # 이미지 경로 저장
            # )               
